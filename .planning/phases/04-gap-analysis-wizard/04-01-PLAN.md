...
must_haves:
  truths:
    - "User progresses through 10 category steps answering 3 questions each (30 total) via 4-level maturity radio buttons"
...
- Both language files have gapAnalysis translations for progress, navigation, maturity option labels, and validation error messages
- Build passes
  </done>
</task>
...
- **Zod schema -- all fields REQUIRED:** Build the schema dynamically so every question must be answered before the user can proceed:
```typescript
...
```
This means if a radio group has no selection, Zod validation fails on submit and the user sees an error. This is correct UX -- they must answer all 3 questions in a category before clicking Next.

- **defaultValues:** Only set values for previously-answered questions. Omit unanswered questions entirely (do NOT set them to `undefined` or `0`):
```typescript
...
```
This way React Hook Form knows which fields have no value yet, and Zod validation correctly rejects incomplete submissions.
...
- **Translation pattern -- use `useTranslations()` with NO namespace at root level:** Question titleKeys are stored as full dotted paths like `questions.raQ1.title`. Use `useTranslations()` (no argument) to get the root translator, then access `t(question.titleKey)` directly. This works because next-intl supports dotted paths from root. Similarly for tooltip keys and maturity description keys. For gapAnalysis UI strings (navigation, labels), use a second `useTranslations('gapAnalysis')` instance or access via `t('gapAnalysis.navigation.back')` from the root translator. Be consistent -- recommended approach:
```typescript
const t = useTranslations(); // root-level for question content (dotted paths)
const tGap = useTranslations('gapAnalysis'); // namespaced for UI chrome
```

- **onSubmit handler:** Convert form data to `Answer[]` array (map entries to `{ questionId, categoryId, level: Number(value) as MaturityLevel }`), call `updateAnswers(categoryAnswers)`. If `isLastCategory`, navigate using `router.push('/{locale}/results')` (import `useRouter` from `next/navigation` -- NOT `window.location.href` which causes a full page reload). Otherwise call `nextCategory()`.
- **onBack handler:** Before navigating back, save current form values (even if incomplete) using `getValues()` from useForm, convert to Answer[] (filtering out entries where value is `undefined`), call `updateAnswers(...)`, then call `prevCategory()`. This prevents data loss on backward navigation.

- **Render each question as a card:**
  - Question title from `t(question.titleKey)` (root translator resolves `questions.raQ1.title`)
  ...
  - **Validation error display:** Below each question's RadioGroup, show the field error if present:
    ```typescript
    {errors[question.id] && (
      <p className="text-sm text-red-600 mt-1">{tGap('validation.required')}</p>
    )}
    ```
...
2. Run `npm run dev`, navigate to `http://localhost:3000/de/gap-analysis`:
   ...
   - Clicking "Weiter" without answering all 3 questions shows validation errors ("Bitte beantworten Sie diese Frage") on unanswered questions
   - Selecting all 3 answers and clicking "Weiter" advances to category 2
...
5. Complete all 10 categories and verify navigation to /results on last step (no full page reload -- SPA navigation via router.push)
6. Partial persistence test: Answer 1 of 3 questions in category 1, click Back then Next -- verify the 1 answer persists
  </verify>
  <done>
...
- All 3 questions must be answered before progressing (Zod validation with per-question error messages)
...
- Last category submit navigates to results page via router.push (SPA navigation, no reload)
