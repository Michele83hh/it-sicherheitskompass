/**
 * PCI DSS v4.0 Quick Check (Schnellcheck)
 *
 * 10 simplified questions for a quick initial assessment of
 * PCI DSS compliance readiness.
 * Answers are yes/partial/no instead of maturity levels.
 *
 * Scoring: yes=100%, partial=50%, no=0%
 * Traffic light thresholds match the full assessment engine.
 *
 * Legal basis: PCI DSS v4.0 (March 2022)
 */

import type { TrafficLight, CategoryScore, OverallScore } from './types';
import { CATEGORIES } from './categories';

export type QuickCheckValue = 'yes' | 'partial' | 'no';

export interface QuickCheckQuestion {
  id: string;
  categoryId: string;
  titleKey: string;
  descriptionKey: string;
}

export interface QuickCheckAnswer {
  questionId: string;
  categoryId: string;
  value: QuickCheckValue;
}

export const QUICK_CHECK_QUESTIONS: QuickCheckQuestion[] = [
  {
    id: 'pci-qc-1',
    categoryId: 'network-security',
    titleKey: 'pciDss.quickCheck.questions.q1.title',
    descriptionKey: 'pciDss.quickCheck.questions.q1.description',
  },
  {
    id: 'pci-qc-2',
    categoryId: 'data-protection',
    titleKey: 'pciDss.quickCheck.questions.q2.title',
    descriptionKey: 'pciDss.quickCheck.questions.q2.description',
  },
  {
    id: 'pci-qc-3',
    categoryId: 'vulnerability-mgmt',
    titleKey: 'pciDss.quickCheck.questions.q3.title',
    descriptionKey: 'pciDss.quickCheck.questions.q3.description',
  },
  {
    id: 'pci-qc-4',
    categoryId: 'access-control',
    titleKey: 'pciDss.quickCheck.questions.q4.title',
    descriptionKey: 'pciDss.quickCheck.questions.q4.description',
  },
  {
    id: 'pci-qc-5',
    categoryId: 'authentication',
    titleKey: 'pciDss.quickCheck.questions.q5.title',
    descriptionKey: 'pciDss.quickCheck.questions.q5.description',
  },
  {
    id: 'pci-qc-6',
    categoryId: 'physical-security',
    titleKey: 'pciDss.quickCheck.questions.q6.title',
    descriptionKey: 'pciDss.quickCheck.questions.q6.description',
  },
  {
    id: 'pci-qc-7',
    categoryId: 'monitoring-testing',
    titleKey: 'pciDss.quickCheck.questions.q7.title',
    descriptionKey: 'pciDss.quickCheck.questions.q7.description',
  },
  {
    id: 'pci-qc-8',
    categoryId: 'security-policy',
    titleKey: 'pciDss.quickCheck.questions.q8.title',
    descriptionKey: 'pciDss.quickCheck.questions.q8.description',
  },
  {
    id: 'pci-qc-9',
    categoryId: 'data-protection',
    titleKey: 'pciDss.quickCheck.questions.q9.title',
    descriptionKey: 'pciDss.quickCheck.questions.q9.description',
  },
  {
    id: 'pci-qc-10',
    categoryId: 'monitoring-testing',
    titleKey: 'pciDss.quickCheck.questions.q10.title',
    descriptionKey: 'pciDss.quickCheck.questions.q10.description',
  },
];

function valueToPercentage(value: QuickCheckValue): number {
  switch (value) {
    case 'yes': return 100;
    case 'partial': return 50;
    case 'no': return 0;
  }
}

function getTrafficLight(percentage: number): TrafficLight {
  if (percentage < 40) return 'red';
  if (percentage < 70) return 'yellow';
  return 'green';
}

function roundToOneDecimal(value: number): number {
  return Math.round(value * 10) / 10;
}

/**
 * Calculate PCI DSS quick check score from answers.
 * Returns the same OverallScore structure as the full assessment.
 */
export function calculateQuickCheckScore(answers: QuickCheckAnswer[]): OverallScore {
  const totalCategories = CATEGORIES.length;

  // Group answers by category and average them (since some categories have 2 questions)
  const categoryScores: CategoryScore[] = CATEGORIES.map((cat) => {
    const categoryAnswers = answers.filter((a) => a.categoryId === cat.id);
    if (categoryAnswers.length === 0) {
      return {
        categoryId: cat.id,
        percentage: 0,
        trafficLight: getTrafficLight(0),
        answeredQuestions: 0,
        totalQuestions: QUICK_CHECK_QUESTIONS.filter((q) => q.categoryId === cat.id).length,
      };
    }
    const percentage = roundToOneDecimal(
      categoryAnswers.reduce((sum, a) => sum + valueToPercentage(a.value), 0) / categoryAnswers.length
    );
    return {
      categoryId: cat.id,
      percentage,
      trafficLight: getTrafficLight(percentage),
      answeredQuestions: categoryAnswers.length,
      totalQuestions: QUICK_CHECK_QUESTIONS.filter((q) => q.categoryId === cat.id).length,
    };
  });

  const answered = categoryScores.filter((cs) => cs.answeredQuestions > 0);
  const overallPercentage = answered.length > 0
    ? roundToOneDecimal(answered.reduce((sum, cs) => sum + cs.percentage, 0) / answered.length)
    : 0;

  return {
    percentage: overallPercentage,
    trafficLight: getTrafficLight(overallPercentage),
    categoryScores,
    answeredQuestions: answers.length,
    totalQuestions: QUICK_CHECK_QUESTIONS.length,
    completionRate: roundToOneDecimal((answers.length / QUICK_CHECK_QUESTIONS.length) * 100),
  };
}
