'use client';

import { useTranslations } from 'next-intl';
import { Link } from '@/lib/i18n/navigation';
import { Truck, ChevronLeft, AlertTriangle, ListChecks, Lightbulb } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { WissenBreadcrumb } from '@/components/layout/breadcrumb';

const WEIGHT_COLORS: Record<string, string> = { Hoch: 'bg-red-100 text-red-700', High: 'bg-red-100 text-red-700', Mittel: 'bg-amber-100 text-amber-700', Medium: 'bg-amber-100 text-amber-700' };

export default function LieferantenAmpelPage() {
  const t = useTranslations('wissenPages.lieferantenAmpel');
  const tWissen = useTranslations('wissenPages');

  return (
    <div>
      <section className="bg-gradient-to-b from-slate-900 to-slate-800">
        <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8 text-center flex flex-col items-center justify-center min-h-[14rem] sm:min-h-[16rem]">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-white/10">
            <Truck className="size-7 text-white" />
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">{t('title')}</h1>
          <p className="mt-4 text-lg text-slate-300">{t('subtitle')}</p>
        </div>
      </section>

      <div className="mx-auto max-w-4xl px-4 py-8 sm:py-12">
        <WissenBreadcrumb />
        <p className="mb-10 text-base leading-relaxed text-muted-foreground">{t('intro')}</p>

        <section className="mb-10">
          <h2 className="mb-4 text-2xl font-bold flex items-center gap-2"><AlertTriangle className="size-6 text-amber-500" />{t('why.title')}</h2>
          <div className="rounded-lg border-2 border-amber-200 bg-amber-50 p-5 space-y-2">
            {['i1', 'i2', 'i3', 'i4', 'i5'].map((id) => (
              <p key={id} className="flex gap-2 text-sm text-amber-900"><AlertTriangle className="mt-0.5 size-4 shrink-0" />{t(`why.items.${id}` as any)}</p>
            ))}
          </div>
        </section>

        <section className="mb-10">
          <h2 className="mb-4 text-2xl font-bold flex items-center gap-2"><ListChecks className="size-6 text-primary" />{t('criteria.title')}</h2>
          <div className="space-y-3">
            {['i1', 'i2', 'i3', 'i4', 'i5', 'i6', 'i7', 'i8'].map((id) => {
              const weight = t(`criteria.items.${id}.weight` as any);
              return (
                <Card key={id}><CardContent className="flex items-start justify-between gap-3 pt-4">
                  <div><p className="font-semibold text-sm">{t(`criteria.items.${id}.title` as any)}</p><p className="text-xs text-muted-foreground mt-0.5">{t(`criteria.items.${id}.desc` as any)}</p></div>
                  <Badge className={`shrink-0 text-xs ${WEIGHT_COLORS[weight] || 'bg-muted text-muted-foreground'}`}>{weight}</Badge>
                </CardContent></Card>
              );
            })}
          </div>
        </section>

        <section className="mb-10">
          <h2 className="mb-4 text-2xl font-bold">{t('ampel.title')}</h2>
          <div className="space-y-3">
            {(['green', 'yellow', 'red'] as const).map((c) => (
              <div key={c} className={`rounded-lg p-4 ${c === 'green' ? 'bg-emerald-50 border border-emerald-200' : c === 'yellow' ? 'bg-amber-50 border border-amber-200' : 'bg-red-50 border border-red-200'}`}>
                <p className={`font-bold text-sm mb-1 ${c === 'green' ? 'text-emerald-700' : c === 'yellow' ? 'text-amber-700' : 'text-red-700'}`}>{t(`ampel.${c}.title`)}</p>
                <p className={`text-xs ${c === 'green' ? 'text-emerald-700' : c === 'yellow' ? 'text-amber-700' : 'text-red-700'} opacity-80`}>{t(`ampel.${c}.desc`)}</p>
              </div>
            ))}
          </div>
        </section>

        <section className="mb-10">
          <h2 className="mb-4 text-2xl font-bold">{t('tips.title')}</h2>
          <div className="rounded-lg border-2 border-emerald-200 bg-emerald-50 p-5 space-y-2">
            {['i1', 'i2', 'i3', 'i4', 'i5'].map((id) => (
              <p key={id} className="flex gap-2 text-sm text-emerald-900"><Lightbulb className="mt-0.5 size-4 shrink-0" />{t(`tips.items.${id}` as any)}</p>
            ))}
          </div>
        </section>

        <Link href="/wissen" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <ChevronLeft className="size-4" /> {tWissen('backToWissen')}
        </Link>
      </div>
    </div>
  );
}
