...
import { FileSearch, ChevronLeft, AlertTriangle, ListChecks, FileWarning, Lightbulb } from 'lucide-react';
import { Card, CardContent } from '@/components/ui/card';
import { WissenBreadcrumb } from '@/components/layout/breadcrumb';

export default function DsfaPage() {
  const t = useTranslations('wissenPages.dsfa');
    ...
    <div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:py-12">
        ...
        <section className="mb-10">
          ...
          <p className="mb-4 text-sm text-muted-foreground">{t('when.desc')}</p>
            ...
            {['s1', 's2', 's3', 's4', 's5', 's6', 's7'].map((id, idx) => (
              <Card key={id}>
                <CardContent className="flex gap-3 pt-4">
                  ...
                  <div>
                    <p className="font-semibold text-sm">{t(`steps.items.${id}.title` as any)}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">{t(`steps.items.${id}.desc` as any)}</p>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <section className="mb-10">
          ...
          <p className="mb-4 text-sm text-muted-foreground">{t('blacklist.desc')}</p>
        ...
        <div className="mb-8 rounded-lg bg-primary/5 border-2 border-primary/20 p-6 text-center">
          <p className="text-sm text-foreground mb-3 font-medium">{tWissen('vvt.title')}</p>
          <Link href="/wissen/vvt" className="inline-flex items-center gap-2 rounded-md bg-emerald-500 px-6 py-2.5 text-sm font-medium text-white shadow-md shadow-emerald-500/20 hover:bg-emerald-600 transition-colors">
            {tWissen('vvt.title')}
          </Link>
        </div>
